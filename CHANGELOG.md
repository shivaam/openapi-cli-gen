...
The 0.0.16 change that makes body fields CLI-optional (so `--root` can bypass them) had an unintended side effect: it stripped `serialization_alias` from fields generated by the simple schema builder (used for multipart bodies). Immich's `assets upload` endpoint broke because `device_asset_id` was serialized as `device_asset_id` instead of `deviceAssetId`, causing the server to reject the upload with "deviceAssetId must be a string".

The fix: check for an existing `serialization_alias` on the field (not just `alias`) and preserve it when rebuilding the FieldInfo.

Caught by re-verifying `immich-rest-cli assets upload` against live Immich before rewriting the README. Regression suite still 36/36.
